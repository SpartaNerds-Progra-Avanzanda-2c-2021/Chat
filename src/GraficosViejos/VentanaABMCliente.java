...
public class VentanaABMCliente extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public VentanaABMCliente(VentanaPrincipal principal) {
	...
	private Cliente cargaDatosCliente() {
		return new Cliente(this.txtDni.getText().trim(), (String) this.cmbTipoDoc.getSelectedItem(),
				this.txtApellido.getText(), this.txtNombre.getText(), this.textArea.getText());
	}
	...
	private void abmCliente() {
		...
		try {
			if (this.btnSalir.getText().equals("Alta")) {
				...
				principal.ventanaPrincipalEventos.agregarNuevoCliente(cli);
				principal.ventanaPrincipalEventos.agregarNuevoClienteGrilla(cli, principal.tblClientes);
			}

			if (this.btnSalir.getText().equals("Modificar")) {
				cli = cargaDatosCliente();
				principal.ventanaPrincipalEventos.modificarCliente(cli);
				principal.ventanaPrincipalEventos.modificarClienteGrilla(cli,
						principal.ventanaPrincipalEventos.obtenerFilaSeleccionadaGrilla(principal.tblClientes),
						principal.tblClientes);
			}

			if (this.btnSalir.getText().equals("Ver"))
				return;

			if (this.btnSalir.getText().equals("Eliminar")) {
				if (JOptionPane.showConfirmDialog(this, "Desea Eliminar el Cliente?", "Confirmar",
						JOptionPane.OK_OPTION) == JOptionPane.OK_OPTION) {
					cli = cargaDatosCliente();
					principal.ventanaPrincipalEventos.eliminarCliente(cli);
					principal.ventanaPrincipalEventos.eliminarClienteGrilla(cli,
							principal.ventanaPrincipalEventos.obtenerFilaSeleccionadaGrilla(principal.tblClientes),
							principal.tblClientes);
				}
			}
			...
			JOptionPane.showMessageDialog(this, this.btnSalir.getText() + ": Ocurió un error.");
		}
